...
- 가장 큰 `Z` 오른쪽 귀퉁이에 원뿔형 몸체와 흰 털공만 가진 잠옷 모자 추가
...
가장 큰 `Z`의 오른쪽 위 귀퉁이를 기준으로 WebGL 모자를 추가했다.
모자는 파란 원뿔형 본체와 흰색 털공 sphere mesh만 남겨 단순화했다.
모자 파트는 모두 `Zzz`와 같은 group matrix 안에서 렌더링되므로, 마우스로 글자를 회전할 때 함께 움직인다.
흰 털공은 본체 원뿔의 실제 tip 좌표에 바로 붙도록 계산했다.

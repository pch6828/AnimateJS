...
export const descriptionJ = [
    `이젠 MBTI는 한물 가긴 했지만, 전 흔히 말하는 "파워 J"입니다.
    어떤 일이던지 미리 계획을 세우는 편이고, 즉흥적인 걸 그닥 선호하지 않아요.`,
    `그렇다 보니 가끔 "너무 피곤하게 산다", "계획이 틀어질 수도 있지" 같은 말을 듣기도 합니다.
    흔히 J는 계획이 틀어지면 스트레스를 받는다고 하니까요.
    그렇지만 제가 그정도로 중증 강박증 환자는 아닙니다.
    애초에 제 계획이 틀어지는 경우는 그리 많지 않아요.
    다양한 상황을 대비해서 플랜 A, B, C... 같은 여러 계획을 세워두는 편이거든요.
    각 계획마다 어느 정도 여유를 남겨둬서 예상치 못한 상황에도 어느정도 대처할 수 있구요.`,
    `다만, 이 "여유"를 남기는 습관이 만드는 문제가 있습니다.
    바로 시간 약속을 지키기 어렵다는 것이죠.
    늦는다는 말은 아니에요. 오히려 너무 빨리 와서 문제죠.
    이동 단계별로 발생하는 변수를 고려하다 보니 거의 2~30분씩 일찍 도착합니다.
    심지어 기차나 버스 같은 대중교통이 중간에 끼면 더더욱 심해지죠.
    나름대로 노력하고 있는데 아직도 이건 잘 안 고쳐지네요.`
];

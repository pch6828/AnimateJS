import {Point} from './point.js?ver=1';
...
export class Dialog{
    ...
    animate(ctx){
        ...
        this.centerPos = this.pos.clone().add(this.moucePos);
    ...
    down(point){
        if(point.collide(this.pos, WIDTH, HEIGHT)){
            ...
            this.moucePos = point.clone().subtract(this.pos);
            
            const xRatioValue = this.moucePos.x/WIDTH;
            this.origin.x = WIDTH*xRatioValue;
            this.origin.y = HEIGHT*this.moucePos.y/HEIGHT;
            return this;
        }else{
            return null;
        }
